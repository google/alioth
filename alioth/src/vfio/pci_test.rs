...
fn test_vfio_pci_dev_creation_and_config() {
    ...
    assert_matches!(config.read(0x00, 2), Ok(0x1ae0)); // Vendor ID
    ...
    // Verify PCI config writes
    assert_matches!(config.write(0x00, 2, 0x1234), Ok(_));
    assert_matches!(config.read(0x00, 2), Ok(0x1ae0)); // Vendor ID
    ...
    // Reset via Pci trait
    assert_matches!(Pci::reset(&dev), Ok(()));
