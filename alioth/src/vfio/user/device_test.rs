...
fn handle_vfio_user_server(mut stream: &UnixStream) {
    ...
    loop {
        ...
        let bytes = match recv_msg_with_fds(stream, &mut header_slice, &mut recv_fds) {
            ...
            Ok(b) => b,
            Err(e) => {
                log::error!("server recv_msg error: {e:?}");
                break;
            }
        };
        if bytes != size_of::<VfioUserHeader>() {
            log::error!("vfio-user server: failed to read a complete header");
            break;
        }
        let payload_size = req_header.msg_size as usize - size_of::<VfioUserHeader>();
        if payload_size > 0
            && let Err(e) = stream.read_exact(&mut payload_buf[..payload_size])
        {
            log::error!("server read_exact payload error: {e:?}");
            break;
        }
