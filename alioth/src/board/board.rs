    ...
    fn run_vcpu_inner(
        ...
        loop {
            ...
            if mp_sync.state == BoardState::Running {
                ...
                for (vcpu_index, (handle, _)) in vcpus.iter().enumerate() {
                    if index != vcpu_index as u16 {
                        log::info!("VCPU-{index}: stopping VCPU-{vcpu_index}");
                        self.vm
                            .stop_vcpu(vcpu_index as u64, handle)
